...
def load_triangle_splats(checkpoint_path: Path):
    ...
    # Auto-detect PLY vs PT by file suffix
    if str(checkpoint_path).lower().endswith(".ply"):
        # Load from PLY (exported by TriangleSerializer.save_to_ply)
        ply = PlyData.read(str(checkpoint_path))
        metadata = {}
        if ply.obj_info:
            try:
                metadata = json.loads("\n".join(ply.obj_info))
        ...
        vertices = points
        vertex_weight = vertex_data["vertex_weight"].astype(np.float32).reshape(-1, 1)
        features_dc = np.column_stack((vertex_data["f_dc_0"], vertex_data["f_dc_1"], vertex_data["f_dc_2"])).astype(np.float32)

        num_rest_features = int(metadata.get("num_rest_features", 0))
        features_rest_flat = np.zeros((points.shape[0], num_rest_features * 3), dtype=np.float32)
        for i in range(num_rest_features * 3):
            field_name = f"f_rest_{i}"
            if field_name in vertex_data.dtype.names:
                features_rest_flat[:, i] = vertex_data[field_name]
        if num_rest_features > 0:
            features_rest = features_rest_flat.reshape(points.shape[0], num_rest_features, 3).astype(np.float32)
        else:
            features_rest = np.zeros((points.shape[0], 0, 3), dtype=np.float32)

        face_data = ply["face"].data
        triangle_indices = np.array(face_data["vertex_indices"].tolist(), dtype=np.int32)
        importance_score = face_data["importance_score"].astype(np.float32) if "importance_score" in face_data.dtype.names else np.zeros((triangle_indices.shape[0],), dtype=np.float32)
        image_size = face_data["image_size"].astype(np.float32) if "image_size" in face_data.dtype.names else np.zeros((triangle_indices.shape[0],), dtype=np.float32)

        sigma = float(metadata.get("sigma", 0.0))
        active_sh_degree = int(metadata.get("active_sh_degree", 0))
        # normalize features_dc shape to (V,1,3)
